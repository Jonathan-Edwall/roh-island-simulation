...
#
# First step: Sort the population ROH-file after genomic coordinates (instead of having it sorted after IID)
# Second step: Merge together overlapping ROH-segments in the sorted population roh-file
#いいいいいいいいいいいいいいいいいいいいいいいいいいいいいいいいいいいいいい


# Defining path to the output directory
bedtools_output_dir=$HOME/results/Bedtools
merged_overlapping_rohs_dir=$bedtools_output_dir/empirical/doi_10_5061_dryad_h44j0zpkf__v20210813/merged
# Creating a directory to store the merged file in, if it does not already exist.
mkdir -p $merged_overlapping_rohs_dir

bedtools sort -i <(tail -n +2 "$population_bed_file") | bedtools merge > $merged_overlapping_rohs_dir/merged_roh_regions.bed 
