...
input_file <- file.path(pop_roh_freq_dir,"sim_1_neutral_model_chr1_ROH_freq.bed")


# data_source <- "empirical"
# pop_roh_freq_dir <- file.path(Coverage_dir,data_source,empirical_dog_breed,"pop_roh_freq")
# 
# # input_file <- file.path(repository_path,"res_test/Bedtools/coverage/empirical/doi_10_5061_dryad_h44j0zpkf__v20210813/pop_roh_freq/german_shepherd_ROH_freq.bed")
...
# 
# 
# ROH_hotspots_df <- incorporate_adjacent_windows(merged_top_1_percent_ROH, data, threshold)
...
## 1.5 Creating the final ROH-hotspots by adding adjacent windows under the hotspot-threshold
...
incorporate_adjacent_windows <- function(merged_df, data_df, threshold) {
  ...
  for (i in 1:nrow(merged_df)) {
    ...
    if (length(data_pos1_index) > 0) {
      ...
      while (index - 1 >= 1) {
        lower_row <- data_df[index - 1, ]
        # if (lower_row$CHR == chrom && lower_row$POS2 == merged_df[i, "POS1"] - 1 && lower_row$FREQUENCY >= new_threshold) {
...
print(ROH_hotspots_df)
```
### 1.5.1 Extracting the ROH-frequencies for the final ROH-hotspot windows
```{r}
# Initialize an empty dataframe to store linked windows
