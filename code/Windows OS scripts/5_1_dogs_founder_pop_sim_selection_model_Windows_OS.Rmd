...
n_generations_mate_selection <-  80

n_indv_breed_formation <- 215
...
if (!dir.exists(variant_position_dir)) {
  # Create the working directory if it doesn't exist
  dir.create(variant_position_dir)
}
...
find_causative_variant <- function(population,SP, allele_copies_threshold,chromosome_end_margin,model_chromosome_bp_length) {
  ...
  candidate_variants <- which(0 < count & count <= allele_copies_threshold)
...
```
# Temp testing of mate selection function
```{r}
# n_generations_mate_selection <- 2
...
Mate_Selection_scenario <- function(selection_scenario_pop,SP, n_generations_mate_selection,n_indv_breed_formation,introduce_mutations, causative_variant,s) {
    ...
    for (gen_ix in 2:(n_generations_mate_selection + 1)) {
        ...
        # Check if half the generations have passed and the allele frequency has dropped 10-fold
        else if ( gen_ix == ceiling(n_generations_mate_selection / 2) && 
                 f_causative[gen_ix-1] < (f_causative[1] / 20) ) {
        ...
        # 
        # # Check if the causative variant is lost (allele frequency drops to 0)
        # if (f_causative[gen_ix] == 0) {
        #     cat("Causative variant lost at generation", gen_ix - 1, "\n")
        #     return(list(f_causative[1:(gen_ix-1)], generations[[gen_ix - 1]])) # Exit early
        # }
        # cat("\nGeneration", gen, "Geno:",geno)  
        ...
        # # Retrieves the genotype of the causative variant for each individual in the population  
        # geno <- pullSegSiteGeno(generations[[gen_ix]], simParam = SP)[, causative_variant]
        # 
        # # Calculate allele frequency of the causative variant
        # f_causative[gen_ix] <- sum(geno) / length(geno) / 2
        # 
        ...
        # if (f_causative[gen_ix] == 0) {
        #     cat("Causative variant lost at generation", gen_ix - 1, "\n")
        #     return(list(f_causative[1:(gen_ix)], generations[[gen_ix]])) # Exit early
        # }
  }
