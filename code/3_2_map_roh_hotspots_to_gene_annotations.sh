...
if [ $canfam_reference_genome = True ]; then
    ...
    #     output_transcript_file="${gene_mapping_output_dir}/${prefix}_transcripts.txt"
    #     output_gene_names_file="${gene_mapping_output_dir}/${prefix}_gene_names.txt"
    #     temp_bed_file="${empirical_breed_roh_hotspots_dir}/${prefix}_temp.bed"
    #     temp_transcript_file="${gene_mapping_output_dir}/${prefix}_transcripts_temp.gtf"

    #     # Convert ROH hotspot file to BED format
    #     awk '{print "chr"$1, $2, $3}' OFS='\t' "$roh_hotspot_file" > "$temp_bed_file"
