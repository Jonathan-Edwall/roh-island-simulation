...
#################################### 
ROH_hotspots_dir <- file.path(results_dir,"ROH-Hotspots/empirical/german_shepherd")
# 
...
ROH_hotspot_allele_freq_dir <- file.path(ROH_hotspots_dir,"hotspots_allele_freq")

empirical_genomewide_allele_freq_dir <- file.path(results_dir,"PLINK/allele_freq/empirical/german_shepherd")
simulated_model_allele_freq_dir <- file.path(results_dir,"PLINK/allele_freq/simulated/neutral_model")
...
output_dir_sweep_test <- file.path(results_dir,"Sweep_test/german_shepherd")
# output_dir_sweep_test <- file.path(results_dir,"temp")


expected_heterozygosity_results_dir <- file.path(results_dir,"expected_heterozygosity")
output_empirical_H_e_dir <- file.path(expected_heterozygosity_results_dir,"empirical/german_shepherd")
# output_empirical_H_e_dir <- file.path(expected_heterozygosity_results_dir,"empirical/german_shepherd/temp")


if (!dir.exists(output_empirical_H_e_dir)) {
  # Create the working directory if it doesn't exist
  dir.create(output_empirical_H_e_dir,recursive = TRUE)
}


# output_simulated_model_H_e_dir <- file.path(expected_heterozygosity_results_dir,"simulated/neutral_model/temp")

output_simulated_model_H_e_dir <- file.path(expected_heterozygosity_results_dir,"simulated/neutral_model")

if (!dir.exists(output_simulated_model_H_e_dir)) {
  # Create the working directory if it doesn't exist
  dir.create(output_simulated_model_H_e_dir,recursive = TRUE)
}
...
for (file in ordered_hotspot_window_bed_files) {
  ...
  for (i in 1:nrow(bed_data)) {
    ...
    row_data <- bed_data[i, ]
    # Create hotspot name
    hotspot_name <- paste("Hotspot_chr", chromosome, "_window_", window_counter, sep = "")
...
# Extract the chromosome number and window number from the file names
chr_numbers <- as.numeric(sub(".*CHR(\\d+)_.*", "\\1", bed_files))
window_numbers <- as.numeric(sub(".*windows_(\\d+)_.*", "\\1", bed_files))
...
for (file in ordered_bed_files) {
  ...
  table_name <- paste("Hotspot_chr", chromosome, "_window_", window, "_allele_freq", sep = "")
  # Create table name
  hotspot_name <- paste("Hotspot_chr", chromosome, "_window_", window, sep = "")
...
# #Print the list of empirical_hotspot_allele_Freq_tables
# print(empirical_hotspot_allele_Freq_tables)
...
for (file in bed_files) {
  ...
  if (use_MAF_pruning == TRUE) {
    ...
    # Remove rows where MAF < min_MAF
    bed_data <- bed_data[bed_data$MAF >= min_MAF, ]
  }
...
for (sim_index in seq_along(simulated_model_allele_freq_tables)) {
  ...
  he_values <- simulated_model_allele_freq_tables[[sim_index]][["H_e_list"]]
