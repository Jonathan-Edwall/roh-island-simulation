...
n_generations_mate_selection <-  40


n_ind_per_selection_sim_generation <- 70
...
snp_density_Mb <- 56.21


disappearance_threshold_value_to_terminate_script <- 20
...
#output_dir_simulation <- file.path(YOUR_GITHUB_ROOT_DIRECTORY, repository_path,"data/raw/simulated/selection_temp")
...
simulation_prune_count_file <- file.path(output_dir_simulation, "pruned_counts/pruned_replicates_count_s_0.8.tsv")
variant_positions_file <- file.path(output_dir_simulation, "variant_position/variant_position_s_0.8.tsv")
...
find_causative_variant <- function(population,SP, allele_copies_threshold,chromosome_end_margin) {
...
last_bottleneck_IID <- integer(max(last_bottleneck_generation@id))
#last_bottleneck_IID <- as.numeric(max(last_bottleneck_generation@id))
...
while (!success) {
  ...
  causative_variant <- find_causative_variant(last_bottleneck_generation, SP, allele_copies_threshold,chromosome_end_margin)
  ...
  if (max(f_causative) >= causative_variant_prune_threshold) {
    ...
    #SP$resetPed(last_bottleneck_IID)
