...
s <- 0.6 # Selection coefficient - 0.05 default
n_ind_per_selection_sim_generation <- 50 # 1000 default
...
n_generations_mate_selection <- 40 # 300?
...
output_sim_files_basename <- paste0("Selection_Scenario_simulation_", chr_simulated)
...
## 2.1: Bottleneck Simulation for 10 Generations - randCross()
...
## 2.3 Choosing a causative variant for the Selection Scenario
...
#candidate_variants <- which(count == allele_copies_threshold) # equals 10

#Randomly picking out one of the candidate variant as the causative variant to be studied
...
for (gen in 1:n_generations_mate_selection) {
    ...
    # Sets the fitness of individuals based on their genotype and the selection coefficient.
    fitness <- rep(1-s, length(geno))
    fitness[geno == 2] <- 1
    fitness[geno == 1] <- 1 - (s/2)
    
    ## Biased sampling of parents based on their fitness.
    # Sample with replacement is used to allow the possibility of multiple matings by the same parent within a generation
    # The probability of picking a parent is equivalent to the relative fitness of the parent
    ...
    ## Crossing parent 1 with parent 2
    # cbind(parent1, parent2) contains information about which parents to mate for each generation
    # Each crossing give 1 offspring
...
# Set the plot title
plot_title <- sprintf("Causative Variant Allele frequency plot - s=%g", s)
...
last_mate_selection_generation <- selection_scenario_pop
...
#pullSnpGeno(last_bottleneck_generation,simParam=SP)
```
