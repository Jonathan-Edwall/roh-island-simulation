...
German_shepherd_autosome_ROH_freq_dir <- file.path(German_shepherd_ROH_hotspots_dir,"Gosling_plots//autosome_roh_freq")
...
setwd(German_shepherd_autosome_ROH_freq_dir)
...
chr_roh_frequencies <- data.frame(file_name = population_ROH_freq_files)
...
# empirical_population_name <- sub("^(.*?)_CHR(0-9)_ROH_freq.*", "//1", population_F_ROH_file)
# Combine population name, number, and file name
chr_roh_frequencies <- data.frame(file_name = neutral_model_ROH_freq_files)
...
setwd(German_shepherd_F_ROH_dir)
...
selection_testing_file <- list.files(pattern = pattern)
