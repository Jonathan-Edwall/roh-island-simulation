...
causative_variant_prune_threshold <- 0.99
...
Mate_Selection_scenario <- function(selection_scenario_pop, n_generations_mate_selection, causative_variant, s, n_ind_per_selection_sim_generation,SP) {
  ...
  generations[[1]] <- selection_scenario_pop # The initial breeding group population gets stored as the first element
    ...
    for (gen_ix in 2:(n_generations_mate_selection + 1)) {
      ...
      parent1 <- integer(n_ind_per_selection_sim_generation)
      parent2 <- integer(n_ind_per_selection_sim_generation)
      
      # Biased sampling of parents based on fitness
      # Create mating pairs
      for (i in 1:n_ind_per_selection_sim_generation) {
          # Sample parent1
