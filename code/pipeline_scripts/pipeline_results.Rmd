...
```
### 1.1.3: Summary - Variant fixation 
...
```
### 1.2.3: Causative variant windows
To see ROH-frequency plots of the created Causative Variant Windows, uncomment the block of code between "Start plot code" and "End plot code"
...
for (selection_coefficient in names(window_H_e_causative_variant_tables)) {
  ...
  # Calculate the standard error-based confidence interval of the point estimate H_e across all 20 simulations for the current selection coefficient
...
```
## 1.4: Summary - Causative variant windows 
...
```
### 2.1.4: Summary - ROH-hotspot threshold 
```{r echo = FALSE,warning = FALSE}
...
print(paste("Standard Error 95% Confidence Interval: [", 
...
for (selection_coefficient in names(F_ROH_tables_Selection_Model)) {
  ...
  # Calculate the Standard Error confidence interval of the point estimate F_ROH across all 20 simulations for the current selection coefficient
...
```
## 4.5: Genomewide 5th percentile comparison - Expected Heterozygosity Summary
...
```
### (Optional) 5.2.2: Sweep Test of Causative Variant Windows
This Optional quality control step verifies whether the Causative Variant Windows in the simulated selection models are accurately identified as regions under selection based on the results of the sweep test.
```{r ,echo = FALSE,warning = FALSE}
...
```
## 5.3: Selection Strength Estimation of Empirical ROH-Hotspots
...
### 5.3.1: Comparison of ROH-Hotspots and Causative Variant Windows
```{r ,echo = FALSE,warning = FALSE}
# Initialize an empty data frame to store the results
...
```
#### 5.3.1.1: Comparison Sorted after the different metrics
```{r }
# Sort the data frame based on average fixation time, in descending order
...
```
### 5.3.2: 3D Plot Comparison of Mean Values  
```{r ,echo = FALSE}
setwd(output_dir)
...
### 5.3.3: Visualization of Confidence Intervals and Means
In this section, plots of the standard error-based confidence intervals, alongside mean values, are created to account for the distribution of values, rather than relying solely on the mean (As in the 3D-plot).
...
#### 5.3.3.1: Standard Error CI for H_e  
```{r echo = FALSE,warning = FALSE}
setwd(output_dir)
...
```
#### 5.3.3.2: Standard Error CI for Window Length
```{r echo = FALSE,warning = FALSE}
setwd(output_dir)
generate_H_e_plot <- function(hotspot_name, empirical_value) {
  ...
  inside_models <- plotting_data$Model[plotting_data$Empirical_Within_CI]
  outside_models <- plotting_data$Model[!plotting_data$Empirical_Within_CI]
  cat("Models where empirical Length is within CI for", hotspot_name, ":\n")
  print(inside_models)
...
```
#### 5.3.3.3: Standard Error CI for ROH Freq
```{r echo = FALSE,warning = FALSE}
setwd(output_dir)
generate_H_e_plot <- function(hotspot_name, empirical_value) {
  ...
  inside_models <- plotting_data$Model[plotting_data$Empirical_Within_CI]
  outside_models <- plotting_data$Model[!plotting_data$Empirical_Within_CI]
  cat("Models where empirical ROH-freq is within CI for", hotspot_name, ":\n")
  print(inside_models)
