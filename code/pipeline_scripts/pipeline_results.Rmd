...
pruned_replicates_count_files <- list.files(pattern = pattern)
...
sim_name <- sub("^(.*?)_causative_variant_window.*", "\\1", pruned_replicates_count_files)
...
# Define the filename with the output directory path
filename <- file.path(output_dir, paste("ROH-hotspot threshold comparison",".csv", sep = ""))
...
pattern <- ".*avg_H_e.*\\.tsv$"
causative_variant_window_H_e_files <- list.files(pattern = pattern)
...
# Define the filename with the output directory path
filename <- file.path(output_dir, paste("Expected Heterozygosity Summary",".csv", sep = ""))
...
# Define the filename with the output directory path
filename <- file.path(output_dir, paste("ROH_hotspots_Selection_testing_neutral_model_H_E_threshold_",formatted_selection_H_e_threshold,".csv", sep = ""))
# Write data to CSV file without quotes
write.table(Selection_testing_results, file = filename, sep = ",", row.names = FALSE, quote = FALSE)
