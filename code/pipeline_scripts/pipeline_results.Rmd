---
title: "Pipeline Summary - No Maf used - N_e 50"
output:
  html_document:
    toc: true
    toc_depth: 3  
...
pattern <- ".*\\.txt$"
causative_variant_fixation_files <- list.files(pattern = pattern)
...
for (i in 1:length(sim_info$sim_name)) {
  ...
  # Read the .txt file into a data frame, using the cleaned column names
  ...
  table_info <- list(Sim_name = simname_from_file, filename = file,fixation_time = fixation_time, 
...
for (selection_scenario in names(fixation_data_causative_variant_tables)) {
  ...
  result_df <- rbind(result_df, data.frame(Selection_coefficient = selection_coefficient, Avg_Fixation_time = Avg_fixation_time, Min_fixation_time = Min_fixation_time,
...
points(matching_row$POS1, variant_100k_window_frequency, col = "cyan", pch = 16)
