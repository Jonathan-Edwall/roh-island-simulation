...
simulation_numbers <- as.integer(sub("^sim_(\\d+)_.*", "\\1", frq_bed_files))
...
for (file in sorted_files) {
  ...
  # Append the table info to the list
  simulated_model_tables <- c(simulated_model_tables, list(table_info))
}


# Print the list of simulated_model_tables
#print(simulated_model_tables)
...
pruned_counts <- numeric(length(simulated_model_tables))
datapoints_before_MAF_filtering <- numeric(length(simulated_model_tables))
#sum(simulated_model_tables[[1]][["data"]][["MAF"]] < 0.05)
#sum(simulated_model_tables[[20]][["data"]][["MAF"]] < 0.05)


for (i in seq_along(simulated_model_tables)) {
  
  
  # Extract table name and data frame
  table_info <- simulated_model_tables[[i]]
  table_data <- table_info$data
...
if (length(rows_to_remove) > 0) {  # Check if rows_to_remove is not empty
  # Remove rows from simulated_model_tables with MAF less than 0.05
  table_data_filtered <- table_data[-rows_to_remove, ]
  simulated_model_tables[[i]]$data <- table_data_filtered
}

    
  
  # Assign the calculated expected heterozygosity list to the H_e attribute of table_info
  simulated_model_tables[[i]]$H_e_list <- window_H_e_list
  simulated_model_tables[[i]]$Marker_based_Average_H_e <- mean(simulated_model_tables[[i]]$H_e_list)
...
for (sim_index in seq_along(simulated_model_tables)) {
  # Get the SNP positions and H_e values for the current simulation
  snp_positions <- simulated_model_tables[[sim_index]][["data"]][["POS1"]]
  he_values <- simulated_model_tables[[sim_index]][["H_e_list"]]
...
image_name <- paste("H_e_distribution_german_shepherd.png")
...
for (i in seq_along(Simulation_windows_avg_he_3_2)) {
  ...
  image_name <- paste(sim_scenario_id,"H_e_distribution_sim_",i,".png")
  ...
  simulated_model_tables[[i]]$H_e_5th_percentile <- percentile_5
...
filename <- file.path(output_simulated_model_H_e_dir, paste(sim_scenario_id,"_5th_percentiles_of_H_e_distribution.tsv", sep = ""))
...
threshold_value <- min(sim_5th_percentiles_df$Fifth_Percentile)


for (i in seq_along(empirical_hotspot_tables)) {
  # Check if Window_based_Average_H_e is <= percentile_5 for the current table (ROH-hotspot)
  if (empirical_hotspot_tables[[i]][["Window_based_Average_H_e"]] <= threshold_value) {
...
# Formatting the threshold value for the output file, to 5 decimals
formatted_H_e_threshold_value <- format(threshold_value, nsmall = 5)

# Define the filename with the output directory path
filename <- file.path(output_dir_sweep_test, paste("ROH_hotspots_", sweep_test_type,"_",sim_scenario_id,"_H_E_threshold_", formatted_H_e_threshold_value, ".tsv", sep = ""))
